...
Key Features:
1)Metadata Compression: Combines multiple data columns into a single searchable context for the AI, ensuring the advisor understands the provider and difficulty of each recommendation.
2)Dynamic Personalization: Adjusts its recommendations based on whether the user identifies as an undergraduate, a working professional, or a hobbyist.
3)Zero-Inference Retrieval: Treats user data as factual constraints to ensure the generated path is grounded in the available catalog.
4)Interactive Menu System: Provides a structured way for users to input their data, reducing errors and improving the quality of the generated advice.
...
1)Data Cleaning: The system cleans the dataset, converting enrollment strings into numerical values and preparing metadata strings.
2)User Input: The agent captures the user's current profile and learning objectives through an interactive loop.
3)Semantic Matching: The system calculates the mathematical similarity between the user's goal and the course catalog to retrieve the top candidates.
4)Contextual Generation: The retrieved courses are passed to the LLM, which generates a 3-step path with detailed justifications for each step.
